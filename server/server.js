  ...
  gmail.users.watch(
    {
      ...
      requestBody: {
        ...
        topicName: 'projects/won-platform-423505/topics/test-gmail-subscription', // Replace with your topic
      },
    },
    (err, res) => {
...
async function fetchRecentEmails(auth, historyId) {
  ...
  try {
    ...
    for (const message of messages) {
      ...
      const fetchedEmailData = await fetchEmailById(auth, message.id, historyId);
...
function getBody(payload) {
  ...
  if (payload.parts) {
    encodedBody = payload.parts.filter(part => part.mimeType === 'text/plain')[0]?.body?.data || '';
  } else {
...
app.post('/createRecord/:tableName', authenticateToken, (req, res) => {
  ...
  try {
    ...
    const sql = `INSERT INTO wonhubs.?? (${columns.join(', ')}) VALUES (${placeholders})`
...
app.post('/testRecord/:tableName', async (req, res) => {
  ...
  if (!tableName || !recordData || typeof recordData !== 'object') {
    return res.status(400).json({ success: false, errors: ['Invalid input data'] });
  }
  ...
  connection.query(schemaQuery, [tableName], (schemaError, schemaResults) => {
    if (schemaError) {
      return res.status(500).json({ success: false, errors: ['Database error: ' + schemaError.message] });
    }
    ...
    for (const column of tableSchema) {
      ...
      if (!column.isNullable && value === undefined && !column.hasDefault && !column.isPrimaryKey) {
      ...
      if (value !== undefined) {
        if (column.type.includes('int') && isNaN(Number(value))) {
          errors.push(`Invalid data type for field '${column.name}': Expected integer.`);
        } else if (column.type.includes('varchar') && typeof value !== 'string') {
          errors.push(`Invalid data type for field '${column.name}': Expected string.`);
        } else if (column.type.includes('datetime') && isNaN(Date.parse(value))) {
          errors.push(`Invalid data type for field '${column.name}': Expected date/time format.`);
        }
      }
    }
    ...
    connection.beginTransaction((err) => {
      if (err) {
        return res.status(500).json({ success: false, errors: ['Database error: ' + err.message] });
      }
      ...
      connection.query(sql, [tableName, ...values], (error) => {
        ...
        connection.rollback(() => {
          ...
          res.json({ success: true, message: 'Validation passed, but no record was inserted.' });
        });
      });
    });
  });
...
app.put('/updateRecord/:tableName/:id', authenticateToken, (req, res) => {
  ...
  console.log("updated record data: ", tableName, !id, !recordData, typeof recordData !== 'object')
  ...
  connection.query(query, [table, id], (err, result) => {
    ...
    if (eventData) {
      ...
      if (table === 'ticket') {
        ...
        createEvent({ ...eventData, title: 'Ticket Deletion Alert', category: 'alert' })
      } else if (table === 'event_logs') {
        createEvent({ ...eventData, title: 'Deleted Record' })
        createEvent({ ...eventData, title: 'Event Record Deletion Alert', category: 'alert' })
      } else {
        createEvent({ ...eventData, title: 'Deleted Record' })
      }
    }
    console.log(`Record with ID ${id} deleted from ${table} successfully.`)
    return res.status(200).json({ message: `Record with ID ${id} deleted from ${table} successfully.` });
  });
})
...
app.post('/get/email-otp', async (req, res) => {
  try {
    ...
    if (eventData) {
      createEvent({ ...eventData, title: `Email OTP has been successfully sent to ${email}.`, category: 'email' })
      createEvent({ ...eventData, title: `Email OTP sent alert.`, category: 'email' })
    }
  } catch (error) {
...
app.delete('/email-log/delete/:emailId', (req, res) => {
  ...
  connection.query(deleteQuery, [recordId], (error, results) => {
    ...
    if (results.affectedRows > 0) {
      return res.json({ success: true, message: 'Email Record deleted successfully' });
    } else {
      return res.status(404).json({ error: 'Email Record not found' });
    }
  });
...
`
  ...
  connection.query(insertQuery, values, (error, results) => {
    if (error) {
      ...
      return res.status(5000).json({ error: 'catalog design internal server Error' })
    }
...
app.get('/catalogDesigns/categories/:department', (req, res) => {
  ...
  const selectQuery = 'SELECT DISTINCT category FROM catalogs WHERE department = ?';

  connection.query(selectQuery, [department], (error, results) => {
    if (error) {
      console.log('Error fetching unique categories of specific department:', error);
      return res.status(500).json({ error: 'Internal server error' });
    }
...
app.get('/catalogDesigns/subCategories/:category/:department', (req, res) => {
  ...
  const selectQuery = 'SELECT DISTINCT sub_category FROM catalogs where department = ? AND category = ?'

  connection.query(selectQuery, [department, category], (error, results) => {
    if (error) {
      console.log('Error fetching unique subCategories of specific department and category:', error);
      return res.status(500).json({ error: 'Internal server error' })
    }
...
app.get('/catalogs/:subCategory/:category/:department', (req, res) => {
  ...
  const selectQuery = 'SELECT * from catalogs where department = ? AND category = ? AND sub_category = ?'

  connection.query(selectQuery, [department, category, subCategory], (error, results) => {
    if (error) {
      console.log('Error fetching unique subCategories of specific department and category:', error);
      return res.status(500).json({ error: 'Internal server error' })
    }
...
app.get('/raiseTicket/:subCategory/:category/:department', (req, res) => {
  ...
  const selectQuery = 'SELECT DISTINCT fields from catalogs where department = ? AND category = ? AND sub_category = ?'

  connection.query(selectQuery, [department, category, subCategory], (error, results) => {
    if (error) {
      console.log('Error fetching fields data of a specific subCategory of specific category of specific deparatment', error);
      return res.status(500).json({ error: 'Internal server error' })
    }
...
`
  ...
  connection.query(insertQuery, values, (error, results) => {
    if (error) {
      ...
      return res.status(5000).json({ error: 'Admin portal form design internal server Error' })
    }
...
if (isProduction) {
  const serverOptions = {
    cert: fs.readFileSync('/etc/letsencrypt/live/api.wonhubs.com/fullchain.pem'),
